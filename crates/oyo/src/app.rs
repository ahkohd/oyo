...
#[derive(Clone, Debug, Hash, PartialEq, Eq)]
struct BlamePrefetchKey {
    path: std::path::PathBuf,
    source: BlameSource,
}

#[derive(Clone, Copy, Debug)]
struct BlamePrefetchRange {
    start: usize,
    end: usize,
}
...
impl App {
    ...
    fn blame_display_from_info(&mut self, info: &BlameInfo, now: i64) -> BlameDisplay {
    ...
    fn blame_uncommitted_display(&self) -> BlameDisplay {
        BlameDisplay {
    ...
    pub(crate) fn prefetch_blame_for_view(
        ...
        let allow_full_fetch = matches!(self.view_mode, ViewMode::Blame);
